...
const BUILTIN_RULES = {
  rules: [
    ...
    },
    {
      ...
      componentFile: "wxml", targets: [],
      patterns: [
        { regex: "<(?!view|text|image|map|button|canvas|scroll-view|collapsible-view|block|template|\\/|!--)[a-zA-Z]", message: "WXML 仅允许 view/text/image/map/button/canvas/scroll-view/collapsible-view 八种组件（含 block/template/注释），不支持 navigator/swiper/input/textarea/picker/checkbox/radio 等" },
        ...
        { regex: "<image[^>]*\\ssrc\\s*=\\s*[\"'][^\"'{]*\\.(?:webp|gif|svg|bmp|avif)\\b", message: "原子组件的 image 仅支持 png / jpg 格式（且仅网络地址）" },
        { regex: "\\sbind(?:touchstart|touchmove|touchend|touchcancel|longpress|longtap|input|change|submit|scroll|blur|focus|confirm)\\s*=", message: "原子组件仅支持 tap 事件与 image 的 load / error 事件，不支持其它交互事件" },
        { regex: "\\sbind:(?:touchstart|touchmove|touchend|touchcancel|longpress|longtap|input|change|submit|scroll|blur|focus|confirm)\\s*=", message: "原子组件仅支持 tap 事件与 image 的 load / error 事件，不支持其它交互事件" },
      ],
    },
    {
      ...
      patterns: [
        ...
        { regex: "display\\s*:\\s*-webkit-box", message: "不支持 display: -webkit-box；多行截断改用 -wx-line-clamp" },
        { regex: "float\\s*:", message: "不支持 float" },
        { regex: "text-decoration\\s*:", message: "不支持 text-decoration" },
        { regex: "--[a-zA-Z][\\w-]*\\s*:", message: "不支持 CSS 变量（--*）" },
        { regex: "(?:^|[;\\{\\s])transition(?:-[a-z]+)?\\s*:", message: "原子组件不支持动画：transition 不在渲染引擎的属性支持范围内，状态变化改为 setData 切类名" },
        { regex: "(?:^|[;\\{\\s])animation(?:-[a-z]+)?\\s*:", message: "原子组件不支持动画：animation 不在渲染引擎的属性支持范围内，状态变化改为 setData 切类名" },
        { regex: "@keyframes\\b", message: "原子组件不支持动画：@keyframes 不可用" },
        { regex: "(?:^|[;\\{\\s])overflow(?:-x|-y)?\\s*:", message: "不支持 overflow / overflow-x / overflow-y：裁剪由宿主容器完成，横向滚动用 <scroll-view scroll-x>；纵向列表可用 collapsible-view（半屏原子组件），只有需要卡片外详情或用户补充信息时才另开半屏页面" },
        { regex: "-webkit-line-clamp\\s*:", message: "不支持 -webkit-line-clamp，改用 -wx-line-clamp（取值 none 或正整数）" },
        { regex: "-webkit-box-orient\\s*:", message: "不支持 -webkit-box-orient；多行截断改用 -wx-line-clamp" },
        { regex: "(?:^|[;\\{\\s])(?:row-|column-)?gap\\s*:", message: "不支持 gap / row-gap / column-gap，用 margin 控制间距" },
        { regex: "cursor\\s*:", message: "不支持 cursor" },
      ],
    },
    {
  ...
  crossFileRules: [
    ...
    { id: "V020", name: "体积与数量上限", stage: "registration", level: "error",
      limits: { skillMd: 16000, agentsMd: 10000, pageMeta: 8000, maxSkills: 30 } },
    { id: "V021", name: "静态原子组件禁用网络请求与定时器", stage: "component", level: "error" },
  ],
};
...
function resolveComponentBase(skillDir, componentPath) {
  if (typeof componentPath !== "string") return null;
  const trimmed = componentPath.trim();
  if (!trimmed || trimmed.startsWith("/") || trimmed.endsWith("/") || trimmed.includes("\\") || trimmed.split("/").includes("..")) return null;
  const skillRoot = resolve(skillDir);
  const base = resolve(skillRoot, trimmed);
  return base !== skillRoot && base.startsWith(skillRoot + sep) ? base : null;
}

async function findDeclaredComponentFiles(skillDir, extension) {
  let mcp;
  try {
    mcp = JSON.parse(await readFile(join(skillDir, "mcp.json"), "utf-8"));
  } catch (error) {
    console.warn(`[components] 无法读取或解析 ${join(skillDir, "mcp.json")}`, error);
    return [];
  }
  const files = new Set();
  for (const item of parseMcpApis(mcp)) {
    const base = resolveComponentBase(skillDir, item?._meta?.ui?.componentPath);
    if (base) files.add(`${base}.${extension}`);
  }
  return [...files];
}
...
async function checkSingleRule(rule, skillsPath, ctx = {}) {
  const files = rule.componentFile
    ? await findDeclaredComponentFiles(skillsPath, rule.componentFile)
    : await glob(rule.targets, skillsPath);
...
const CROSS_CHECKERS = {
  ...
  V020: checkV020_SizeLimits,
  V021: checkV021_StaticComponentRuntime,
};
...
async function checkV012_ComponentFiles(rule, skillsPath) {
  const out = [];
  const extensions = rule.extensions ?? ["js", "json", "wxml", "wxss"];
  for (const skillDir of await findSkillDirs(skillsPath)) {
    const rel = relative(skillsPath, skillDir);
    let apis;
    try {
      apis = parseMcpApis(JSON.parse(await readFile(join(skillDir, "mcp.json"), "utf-8")));
    } catch (error) {
      console.warn(`[V012] 无法读取或解析 ${rel}/mcp.json`, error);
      continue;
    }
    for (const item of apis) {
      ...
      const componentBase = resolveComponentBase(skillDir, compPath);
      if (!componentBase) {
        out.push(fail(rule, `接口 '${name}' componentPath '${compPath}' 必须是相对 skill 目录的组件基路径`, {
          file: `${rel}/mcp.json`,
          fix: `改为相对 skill 目录的组件基路径，例如 "components/<name>/index"`,
        }));
        continue;
      }
      const missing = [];
      for (const extension of extensions) if (!await exists(`${componentBase}.${extension}`)) missing.push(`${compPath}.${extension}`);
      if (missing.length) out.push(fail(rule, `接口 '${name}' 组件 '${compPath}' 缺少：${missing.join(", ")}`, { file: `${rel}/mcp.json`, fix: `补齐 ${missing.join(", ")}` }));
...
async function checkV015_ComponentRelatedPage(rule, skillsPath, ctx = {}) {
  ...
  if (ctx.projectRoot) {
    try {
      ...
      console.warn(`[V015] 读取 app.json 失败，跳过页面存在性校验: ${e.message}`);
    }
  }
  ...
  for (const skillDir of await findSkillDirs(skillsPath)) {
    ...
    const mcpRel = `${rel}/mcp.json`;
    let mcp;
    try { mcp = JSON.parse(await readFile(join(skillDir, "mcp.json"), "utf-8")); }
    catch (e) { console.warn(`[V015] 跳过 ${rel}: mcp.json 读取/解析失败: ${e.message}`); continue; }
    ...
    for (const item of parseMcpApis(mcp)) {
      ...
      if (!rp) {
        out.push(fail(rule, `组件 '${cp}' 在 mcp.json.components[] 中 relatedPage 缺失或为空（卡片右上角"进入小程序"入口为必填项，缺失会在 preview/上传时被平台强校验拦下）`, {
          file: mcpRel,
      ...
      const rpPath = rp.replace(/^\/+/, "").split("?")[0];
      if (appPages && !appPages.includes(rpPath)) {
...
async function checkV020_SizeLimits(rule, _skillsPath, ctx = {}) {
  const out = [];
  const limits = rule.limits ?? {};
  const maxSkillMd = limits.skillMd ?? 16000;
  const maxAgentsMd = limits.agentsMd ?? 10000;
  const maxPageMeta = limits.pageMeta ?? 8000;
  const maxSkills = limits.maxSkills ?? 30;

  for (const skillDir of (ctx.skillDirs ?? [])) {
    const rel = ctx.projectRoot ? relative(ctx.projectRoot, skillDir) : skillDir;
    const mdPath = join(skillDir, "SKILL.md");
    let raw;
    try { raw = await readFile(mdPath, "utf-8"); }
    catch { continue; } // 文件不存在由 V014 负责
    const bytes = Buffer.byteLength(raw, "utf-8");
    if (bytes > maxSkillMd) {
      out.push(fail(rule, `${rel}/SKILL.md ${bytes} 字节，超过上限 ${maxSkillMd}`, {
        file: `${rel}/SKILL.md`,
        fix: `精简 SKILL.md（只保留：能力域定位 / 触发场景 / 不适用范围 / 前置条件 / 使用顺序 5 节；接口契约写在 mcp.json，不要在此重复）；当前超出 ${bytes - maxSkillMd} 字节。SKILL.md 只支持单文件，不能拆分引用其它 md`,
      }));
    } else {
      out.push(pass(rule, `${rule.name}: ${rel}/SKILL.md ${bytes}/${maxSkillMd} 字节`));
    }
  }

  if (!ctx.projectRoot) return out;
  let appJson;
  try { appJson = JSON.parse(await readFile(join(ctx.projectRoot, "app.json"), "utf-8")); }
  catch (e) { console.warn(`[V020] 读取 app.json 失败，跳过项目级上限校验: ${e.message}`); return out; }

  const skills = Array.isArray(appJson?.agent?.skills) ? appJson.agent.skills : [];
  if (skills.length > maxSkills) {
    out.push(fail(rule, `app.json 的 agent.skills 声明了 ${skills.length} 个 SKILL，超过上限 ${maxSkills}`, {
      file: "app.json",
      fix: `合并职责相近的 SKILL，使总数不超过 ${maxSkills}`,
    }));
  } else if (skills.length > 0) {
    out.push(pass(rule, `${rule.name}: agent.skills ${skills.length}/${maxSkills} 个`));
  }

  const extras = [
    { field: "instruction", value: appJson?.agent?.instruction, max: maxAgentsMd, label: "全局提示词" },
    { field: "pageMetadata", value: appJson?.agent?.pageMetadata, max: maxPageMeta, label: "页面元数据" },
  ];
  for (const { field, value, max, label } of extras) {
    if (typeof value !== "string" || !value.trim()) continue; // 未配置则跳过
    const relPath = value.trim().replace(/^\/+/, "");
    const abs = join(ctx.projectRoot, relPath);
    let raw;
    try { raw = await readFile(abs, "utf-8"); }
    catch (e) {
      out.push(fail(rule, `app.json 的 agent.${field} 指向 '${value}'，但该文件读取失败：${e.message}`, {
        file: "app.json",
        fix: `创建 ${relPath}，或移除 app.json 中的 agent.${field} 字段`,
      }));
      continue;
    }
    if (field === "pageMetadata") {
      try { JSON.parse(raw); }
      catch (e) {
        out.push(fail(rule, `${relPath} JSON 解析失败：${e.message}`, { file: relPath, fix: "修复 JSON 语法" }));
        continue;
      }
    }
    const bytes = Buffer.byteLength(raw, "utf-8");
    if (bytes > max) {
      out.push(fail(rule, `${relPath}（${label}）${bytes} 字节，超过上限 ${max}`, {
        file: relPath,
        fix: `精简内容至 ${max} 字节以内，当前超出 ${bytes - max} 字节`,
      }));
    } else {
      out.push(pass(rule, `${rule.name}: ${relPath} ${bytes}/${max} 字节`));
    }
  }
...
async function collectComponentModuleFiles(entryPath, skillDir) {
  const skillRoot = resolve(skillDir);
  const modules = new Map();

  const resolveLocalModule = async (importerPath, spec) => {
    const raw = resolve(dirname(importerPath), spec);
    const candidates = raw.endsWith(".js") ? [raw] : [raw, `${raw}.js`, join(raw, "index.js")];
  ...
  const visit = async (filePath) => {
    ...
    try {
      ...
      console.warn(`[V021] 无法读取组件依赖 ${absPath}`, error);
      return;
    }
    modules.set(absPath, body);
    const specs = new Set([
      ...[...body.matchAll(/\brequire\s*\(\s*['"](\.[^'"]+)['"]\s*\)/g)].map(match => match[1]),
      ...[...body.matchAll(/\bfrom\s*['"](\.[^'"]+)['"]/g)].map(match => match[1]),
    ]);
    for (const spec of specs) {
      if (/\.(?:json|wxml|wxss)$/i.test(spec)) continue;
      const modulePath = await resolveLocalModule(absPath, spec);
      if (!modulePath) {
        console.warn(`[V021] 无法解析组件依赖 '${spec}'（来自 ${absPath}）`, new Error(`未找到 ${spec} 对应的 JavaScript 文件`));
        continue;
      }
      if (!(modulePath === skillRoot || modulePath.startsWith(skillRoot + sep))) {
        console.warn(`[V021] 组件依赖超出 skill 目录 '${spec}'（来自 ${absPath}）`, new Error(`解析结果为 ${modulePath}`));
        continue;
      }
...
async function checkV021_StaticComponentRuntime(rule, skillsPath) {
  const out = [];
  const dynamicOnly = [
    { regex: /\bwx\s*\.\s*login\s*\(/, what: "wx.login" },
    { regex: /\bwx\s*\.\s*checkSession\s*\(/, what: "wx.checkSession" },
    { regex: /\bwx\s*\.\s*request\s*\(/, what: "wx.request" },
    { regex: /(?<![.\w])setTimeout\s*\(/, what: "setTimeout" },
    { regex: /(?<![.\w])setInterval\s*\(/, what: "setInterval" },
  ];
  const cloud = { regex: /\bwx\s*\.\s*cloud\b/, what: "wx.cloud.*" };
  ...
  for (const skillDir of await findSkillDirs(skillsPath)) {
    ...
    try {
      ...
      console.warn(`[V021] 无法读取或解析 ${rel}/mcp.json`, error);
      continue;
    }
    ...
    const dynamicPaths = new Set(
      components
        .filter(c => c && typeof c.path === "string" && c.permissions && c.permissions["scope.dynamic"])
        .map(c => c.path)
    );
    ...
    for (const item of parseMcpApis(mcp)) {
      const cp = item?._meta?.ui?.componentPath;
      if (!cp) continue;
      const componentBase = resolveComponentBase(skillDir, cp);
      if (!componentBase) continue;
      const modules = await collectComponentModuleFiles(`${componentBase}.js`, skillDir);
      if (modules.size === 0) continue;

      const cloudFiles = [...modules]
        .filter(([, body]) => cloud.regex.test(body))
        .map(([filePath]) => relative(skillsPath, filePath));
      const dynamicHits = dynamicOnly
        .filter(f => [...modules.values()].some(body => f.regex.test(body)))
        .map(f => f.what);
      const file = relative(skillsPath, `${componentBase}.js`);

      if (cloudFiles.length) {
        out.push(fail(rule, `组件 '${cp}' 使用了 ${cloud.what}，原子组件环境不支持云开发`, {
          file: cloudFiles[0],
          fix: "将云开发调用移到原子接口；组件及其依赖模块只接收 NotificationType.Result 下发的数据",
        }));
      }
      if (dynamicPaths.has(cp)) {
        if (!cloudFiles.length) out.push(pass(rule, `${rule.name}: ${rel} 组件 '${cp}' 已声明 scope.dynamic`));
        continue;
      }
      if (dynamicHits.length) {
        out.push(fail(rule, `组件 '${cp}' 使用了 ${dynamicHits.join(" / ")}，但未声明 scope.dynamic`, {
          file,
          fix: `去掉这些调用（数据由原子接口经 NotificationType.Result 下发）；确有实时需求时在 mcp.json 的 components[] 该条目加 "permissions": { "scope.dynamic": { "desc": "<使用场景>" } }（需单独审核）`,
        }));
      } else if (!cloudFiles.length) {
        out.push(pass(rule, `${rule.name}: ${rel} 组件 '${cp}' 无动态能力调用`));
      }
    }
  }
  return out;
...
async function validate(projectPath, opts = {}) {
  ...
  const projectLevelRuleIds = new Set(["V016", "V019", "V020"]);
  for (const skillDir of skillDirs) {
    const pkgRoot = packageRootOf(skillDir);
  ...
  for (const r of localRuleSet.crossFileRules) {
    if (!projectLevelRuleIds.has(r.id)) continue;
    all.push(...await checkCrossFileRule(r, null, { projectRoot, skillDirs }));
  }

...
2. 按问题类型改（只改 `components/<name>/`，不动 `mcp.json` / `apis/*`）：
   - **被裁剪（`[ai-mode] ... overflow overflowed=true` 或 `data.overflowHeight > 0`）**：`index.wxss` 压缩 item 高度、用 `-wx-line-clamp:1~2` 截断长文本；根节点**不要写 `overflow` / `max-height` / `min-height` / `height`**（外层尺寸与裁剪都由宿主施加，组件自行设高会让 `NotificationType.Overflow` 回调失效）；单纯列表数据超量时可用半屏原子组件 `collapsible-view` 让卡片自行展开，不要误改成另开半屏页面；只有需要卡片外详情或用户补充信息时才用独立半屏页面。否则在 `index.js` 计算 `visibleItems` + `omittedCount = total - visibleItems.length`，WXML 渲染"还有 {{omittedCount}} 条未展示"
   ...
   - **组件上行协议违规**（点击"活"按钮但小程序 AI 拿不到下一跳、或组件自己调业务接口）：上行合法形态有两种：① 单 `text`（自然语言 followUp）；② `text` + `api/call` 组合（结构化 toolCall）——
     ...
     违规形态包括：`content` 只含 `api/call` 不含前导 `text`（缺用户上下文，小程序 AI 拿不到意图描述）、缺 `content` 数组直接 `{ type: 'api/call', ... }`、`name` 在 mcp.json 中不存在、`arguments` 字段名错或带占位值、组件内直接 `wx.request` 业务接口。`setRelatedPage` 使用在 `created` 中保存的 `this._viewCtx` 是合法形式，不应按缓存引用报错。

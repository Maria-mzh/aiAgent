...
| ⑤ | 长文本省略生效 | `elementTree` 里可 `grep` 到 `…` 或被 `-wx-line-clamp` 压缩的文本；同时核对 ③ 判定未裁剪（存在 `monitor=on`、不出现 `overflowed=true`） | 长文本末尾 `…`，无横向溢出（辅助信号） | 给长文本加 `-wx-line-clamp`（多行）或 `white-space: nowrap` + `text-overflow: ellipsis`（单行）省略样式 |

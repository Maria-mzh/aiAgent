...
description: Use when users need visual direction, interface hierarchy, layout decisions, design specifications, or prototypes before implementing a Web or mini program UI.在实现网页或小程序用户界面之前，当用户需要视觉指导、界面层次结构、布局决策、设计规范或原型时使用。
version: 2.26.0
alwaysApply: false
---

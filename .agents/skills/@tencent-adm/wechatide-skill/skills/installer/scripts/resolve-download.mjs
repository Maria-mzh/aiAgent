...
function isAboveMinimumVersion(version) {
  const number = normalizeVersionNumber(version)
  return number !== null && number > MIN_DOWNLOAD_VERSION_NUMBER
}
...
function compareVersions(left, right) {
  const leftNumber = normalizeVersionNumber(left)
  const rightNumber = normalizeVersionNumber(right)
...
async function selectDownload(urls, preferredTypes, options) {
  const requirePkg = isMacPlatform(options.platform)

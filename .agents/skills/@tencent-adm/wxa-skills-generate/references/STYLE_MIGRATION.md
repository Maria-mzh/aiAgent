...
`references/COMPONENT_TEMPLATES.md` 中的 WXML/WXSS 只是**结构骨架**。模板里的配色（`#ff4d4f`/`#07c160`）、字号（4vw/3.47vw）、圆角（1.07vw）、图片尺寸、字段名（`imageUrl`/`title`/`price`）均为占位示例，**不得原样复制**。
...
| 滚动容器（`scroll-view`） | 仅当源页面是横滚（`scroll-x` / `scrollX`）时保留为 `<scroll-view scroll-x="true">`；容器不支持纵向滚动，纵向内容按 `ATOMIC_COMPONENT_DESIGN.md` §7.2 决策处理 |

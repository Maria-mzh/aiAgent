...
> 高度预估判定溢出且换档/换成半屏原子组件仍无法容纳时，需要使用半屏页面（`ATOMIC_COMPONENT_DESIGN.md` §7.2）。卡片内展示摘要/前 N 项 + "查看全部/详情"按钮，点击 `viewCtx.openDetailPage()` 打开半屏。其他场景仅当业务确有"详情/补充信息"语义时挂上。
...
半屏页面执行环境与小程序页面一致，**但禁用以下任何"会让用户跳出半屏"的能力**。命中即视为不可迁移，按 SKILL.md D.9 判定规则处理（删除调用 / 用网络请求替代 / 触发阻断）。
...
| 维度 | 原子接口 | 原子组件 | 半屏原子组件 `collapsible-view` | 半屏页面 |
|---|---|---|---|---|
| 运行时上下文 | service worker 端 | 渲染层（含 `this`） | 仍是原子组件渲染层 | 普通小程序页面 |
| 本质 | 业务执行能力 | 对话中的 GUI 卡片 | GUI 卡片自身展开成半屏，不是新页面 | 另开一个小程序页面 |
| 调起 `openDetailPage` | ❌ 不可（无 `this` / viewCtx） | ✅ 可（`getViewContext(this)`） | 不需要 | — |
| 调 `sendFollowUpMessage` | ❌ 不可 | ✅ `getContext(this)` | ✅ `getContext(this)` | ✅ `getContext()` 不传 this |
| 默认是否生成 | ✅ 必生成 | 按需（有 UI 时生成） | 列表需要展示更多内容时优先使用 | 默认不生成，仅确有卡片外详情或用户补充信息时按本指南挂上 |

---

## 5. 自检

- [ ] 单纯列表装不下时，可使用半屏原子组件 `collapsible-view` ；没有把它误当成独立半屏页面
- [ ] 仅在需要展示卡片外详情或需要用户补充信息时，才挂本文的独立半屏页面；卡片内有摘要 + "查看详情"入口

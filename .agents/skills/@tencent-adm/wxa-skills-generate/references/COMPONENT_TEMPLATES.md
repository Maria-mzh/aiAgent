...
**支持的事件**：仅支持 `tap` 点击，以及 `image` 的 `load` / `error` 两个事件。其它交互事件（`touch*` / `input` / `change` / `scroll` 等）一律不支持。

**支持的内置组件**：`view`（支持 `hover-class`）、`text`（不支持 `user-select`）、`image`（**仅网络地址，且仅 png、jpg 格式**）、`map`（不支持拖动、放大交互）、`button`（**不含 `open-type`**）、`canvas`（**仅 2d**）、`scroll-view`（**仅横滚 `scroll-x`，禁 `scroll-y`**）、`collapsible-view`（**半屏原子组件**，详见下方"半屏原子组件"节）。**不支持** `swiper` / `input` / `textarea` / `picker` / `checkbox` / `radio` / `form` / `slider` / `switch` / `editor` / `rich-text` / `navigator` / `web-view` / `movable-*` / `root-portal` 等其他内置组件。

**`button` 特殊约束**：不可使用 `open-type`。源代码里的 `<button open-type="share">` 等必须改为 `<button bindtap="...">` + 在 tap handler 中调用 `wx.shareAppMessage` 等对应 JSAPI；手机号相关（`getPhoneNumber` / `getRealtimePhoneNumber`）只能在原子接口侧完成，组件侧不可用。

**普通组件不支持实时动态能力**：`wx.request`、`wx.login`、`wx.checkSession`、`setTimeout` / `setInterval` 需在 `mcp.json` 的 `components[]` 声明 `permissions["scope.dynamic"]`（需单独审核，非必要不用）。`wx.cloud.*` 在任何原子组件中都不可用，数据由原子接口取好后经 `NotificationType.Result` 下发。

**不支持动画**：CSS 的 `transition` / `animation` 不在卡片渲染引擎的属性支持范围内，`wx.createAnimation` 同样不可用。状态变化直接 `setData` 切换类名。

**渲染容器约束**：宽度随屏幕宽度变化，最小高度为宽高比 4:1，最大高度为宽高比 1:1，不超出范围时随内容自动撑高。**初始化时即确定卡片高度，之后不可再改**；**不支持纵向滚动，超出裁剪**（横向超长内容可用 `<scroll-view scroll-x="true">` 包裹；纵向装不下的形态决策见 `ATOMIC_COMPONENT_DESIGN.md` §7.2）。**外层容器尺寸由宿主自动设置，组件根节点不要再写 `max-height` / `min-height` / `height`**（原因见 `ATOMIC_COMPONENT_CSS.md` "渲染容器尺寸"节）。**卡片比例必须从 `1:1 / 4:3 / 16:9 / 3:1 / 4:1` 五档中选一**（见 `ATOMIC_COMPONENT_DESIGN.md` 第一章）；实现层细节见 `ATOMIC_COMPONENT_CSS.md`。

**组件不可声明为虚拟组件**。

**关联小程序页面是必填项**：`mcp.json` 的 `components[]` 里每条都要有 `relatedPage`（卡片右上角"进入小程序"入口），运行时用 `setRelatedPage({ query })` 附加参数。详见下方"关联小程序页面"节。
...
6. **上下文调用规则**——在 `methods` / tap handler / 异步回调里，`sendFollowUpMessage` / `getDimensions` / `updateModelContext` 按使用点获取对应上下文；`setRelatedPage` 可在 `created` 中将 `wx.modelContext.getViewContext(this)` 保存为 `this._viewCtx` 后调用，也可按使用点获取。不要把 `this._viewCtx.setRelatedPage(...)` 当作缓存引用违规。按需调用的卡片过期 API（`wx.modelContext.expireAllCards()` / `getViewContext(this).expirePreviousCards()`，详见 `SKILL.md` D.4）仍在使用点获取上下文。
...
| "换一换" / "换一批" | 同当前展示能力的检索接口（如 `searchItems`） | 带上一轮 query + `{ refresh: true }`（仅当接口 inputSchema 有该字段）。**注意**："查看更多/全部"可用 `collapsible-view` 自行展开，不走上行 `api/call`；只有需要展示卡片外详情或收集用户补充信息时，才调 `viewCtx.openDetailPage()` 另开半屏页面（见 `ATOMIC_COMPONENT_DESIGN.md` §7.3） |
...
卡片标题栏右上角有"进入小程序"入口，靠 `mcp.json` 的 `components[].relatedPage` 配置，**缺失会在预览 / 上传时被平台强校验拦下**。在 `created` 中保存 `this._viewCtx` 后，运行时用 `this._viewCtx.setRelatedPage` 把当前卡片的业务参数拼进 query，让用户点进去就落到正确的数据上。通过该入口进小程序的场景值为 1442 / 1443。
  ...
  lifetimes: {
    created() {
      ...
      modelCtx.on(NotificationType.Result, (data) => {
        ...
        if (!sc || !sc.orderId) return
        this.setData({ orderId: sc.orderId })
        // 只传 query 时，路径取 mcp.json 里 components[].relatedPage
        this._viewCtx.setRelatedPage({ query: `orderId=${sc.orderId}` })
        console.info(`[ai-mode] {componentName} setRelatedPage query=orderId=${sc.orderId}`)
      })
    },
  },
  methods: {
    onSpecChange(e) {
      ...
      // 状态变化后更新 query；组件可能关联多个页面时才额外传 path
...
### 同步交互状态给模型（`updateModelContext`，按需）

用户在卡片上改了选择（选规格、选时段、勾选项等）时，把这个变化同步给模型，模型才能在后续回合里知道用户的最新选择。**与 `sendFollowUpMessage` 的区别**：`sendFollowUpMessage` 会像用户发了一条消息一样推进对话；`updateModelContext` 只更新上下文、不推进对话。

```js
methods: {
  onOptionSelect(e) {
    const { option } = e.currentTarget.dataset
    this.setData({ selected: option })
    wx.modelContext.getViewContext(this).updateModelContext({
      content: [{ type: 'text', text: `用户选择了${option}` }],
    })
    console.info(`[ai-mode] {componentName} updateModelContext text=用户选择了${option}`)
  },
}
```

约束：**必须在用户 tap 手势的回调里同步调用**（不可放在异步逻辑或定时器中）；短时间重复调用会被节流拒绝；`content` 必须是非空数组，每个块的 `text` 不能为空；当前只支持 `text` 类型。

### 半屏原子组件 `collapsible-view`（列表类内容适用）

> **归属**：这是**原子组件自身**的一种形态——卡片可展开成半屏以容纳更多内容，展开后**仍然是原子组件**。它与下方"半屏页面跳转"（`openDetailPage` 另开一个小程序页面）是两件事，不要混用。

列表在卡片里放不下时，可用 `collapsible-view`，比"只显示前 N 条 + 查看全部跳半屏页"更轻——不用新建页面，也不用把数据经 storage 传出去。行为：在卡片形态下按可用空间自动计算能展示多少内容、超出部分直接隐藏；在半屏形态下全部展示。`slot="button"` 用于自定义展开按钮，半屏形态下会自动隐藏。

```xml
<collapsible-view>
  <!-- 推荐：每一项都是 collapsible-view 的直接子节点，空间不足时能按项正确隐藏 -->
  <view wx:for="{{items}}" wx:key="id"
        class="item-card" hover-class="item-card-hover"
        bindtap="onTapItem" data-id="{{item.id}}" data-name="{{item.name}}">
    <view class="item-name">{{item.name}}</view>
    <text class="item-price">¥{{item.price}}</text>
  </view>

  <view slot="button" class="expand-btn" hover-class="expand-btn-hover">查看全部</view>
</collapsible-view>
```

```xml
<!-- 不推荐：外面套一层 view，空间不足时整块一起被隐藏 -->
<collapsible-view>
  <view>
...
> 前述所有限制（事件、内置组件、CSS、无动画、无定时器）在半屏形态下依旧适用；卡片的宽高比档位约束只作用于未展开的卡片形态。
...
> 与 `collapsible-view` 的区别：半屏页面是指**另开一个小程序页面**，运行环境等同普通小程序页面（限制见 `HALF_SCREEN.md`）。仅当需要展示卡片里没有的详情、或让用户补充信息时才用；单纯"列表放不下想看全部"用 `collapsible-view`。
>
> 入口仅在组件 `methods` 内（原子接口无 `this`，拿不到 viewCtx）。完整 API 见 `references/HALF_SCREEN.md`。
...
> 纵向内容装不下时：完整展示现有卡片内容可使用半屏原子组件 `collapsible-view` （卡片自身形态变化，见上方"半屏原子组件"节）；需要卡片外详情或用户补充信息时使用下方方式一（截断 top-N + 跳独立半屏页面）；横向排列型内容用方式二。

#### 方式一：截断 top-N + 跳半屏页面（纵向内容备选）
...
Component({
  ...
  methods: {
    ...
    // 备选：只有需要卡片外详情或用户补充信息时，查看全部才打开独立半屏页面
    onTapViewAll() {
      const viewCtx = wx.modelContext.getViewContext(this)
...
- [ ] **高度预估与溢出**：已按 §7 预估；溢出时优先换档，换档仍溢出时，纵向列表可生成半屏原子组件 `collapsible-view` ，横向内容用 `scroll-x`；需要卡片外详情或用户补充信息时另开半屏页面
...
- [ ] 除已声明 `scope.dynamic` 的实时请求外，业务数据均由原子接口取好后下发；组件内不调用 `wx.cloud.*`
...
- [ ] `mcp.json` 的 `components[]` 里本组件条目已填 `relatedPage`（必填），且运行时在收到 `Result` 后调了 `setRelatedPage({ query })`
- [ ] 卡片上有"改变选择"类交互（选规格 / 选时段 / 勾选）时，已在 tap handler 内调 `updateModelContext` 同步给模型
- [ ] wxml 里没有用到不支持的内置组件；`image` 只用网络地址且是 png / jpg
- [ ] wxss 里没有 `transition` / `animation`（不支持动画）、没有 `overflow-y`（禁纵向滚动）
- [ ] `setTimeout` / `setInterval` / `wx.request` / `wx.login` / `wx.checkSession` 仅在该组件已声明 `scope.dynamic` 时使用
...
Component({
  ...
  lifetimes: {
    created() {
      ...
      // 获取视图上下文：关联页面 + 容器尺寸 + 溢出监听
      this._viewCtx = wx.modelContext.getViewContext(this)
      const viewCtx = this._viewCtx
      const { minHeight, maxHeight, width } = viewCtx.getDimensions()
      console.info(`[ai-mode] {componentName} dimensions width=${width} minHeight=${minHeight} maxHeight=${maxHeight}`)
      ...
      // `this._viewCtx` 可供后续 `setRelatedPage({ query })` 使用；已注册的 on(...) 回调由 SDK 持有。
    }
  }
})
```

...
> ⚠️ 以下是技能分包**白名单内**支持的 JSAPI 搜索模式。**完整白名单见 `references/JSAPI_WHITELIST.md`**（`SKILL.md` D 节只列高频项）。源项目中可能用到白名单之外的接口（如 `wx.navigateTo`、`wx.showModal`），这些不可迁移，提取时应标记并按 D.7 / D.9 判定规则处理。
...
> `wx.openBusinessView` 的 `businessType` 仅使用官方 API 支持列表列出的值；原子接口负责返回支付参数，原子组件负责调用支付 API。生成前对照 `references/JSAPI_WHITELIST.md`。
...
> `wx.previewMedia`、`wx.chooseMedia`、`wx.chooseMessageFile` 都在原子组件侧使用，原子接口不支持。
...
wx\.chooseAddress\s*\(
wx\.chooseInvoice\s*\(
wx\.chooseInvoiceTitle\s*\(
wx\.saveImageToPhotosAlbum\s*\(
wx\.scanCode\s*\(
```

这些能力在原子组件侧可用，不需要声明 `scope.dynamic`；调用时机按各 API 的官方说明处理。
...
追踪返回的 `UploadTask` / `DownloadTask` 方法。
...
wx\.checkIsSupportFacialRecognition\s*\(
wx\.requestFacialVerify\s*\(
```
...
`wx.env`、`wx.getDeviceInfo`、`wx.getAppBaseInfo`、`wx.getWindowInfo` 接口与组件**均可用**；`wx.getUserCryptoManager` 仅接口可用。
...
```regex
wx\.(chooseImage|chooseVideo)\s*\(
wx\.(setClipboardData|getClipboardData|previewImage)\s*\(
...
遇到源项目中使用了这些接口的业务功能时，按 `references/JSAPI_WHITELIST.md` §3/§4 的替代策略与判定规则处理。

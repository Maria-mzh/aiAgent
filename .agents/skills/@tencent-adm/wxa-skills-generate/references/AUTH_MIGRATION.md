...
鉴权 / 登录 / 通用参数 / 签名 / `request.js` 复刻的唯一规则源。本文件同时定义 **`analysis-auth.md`（阶段 1.2 产物）两节——鉴权事实节与逐字代码片段节——的填写指引**。
...
| 类型 | 例子 | 载体 |
|------|------|------|
| **离散事实** | 登录 URL、请求/响应字段名、响应取值路径层级、host 映射、哪个 header 何时生效 | **结构化**（§2 的表格 / analysis-auth.md 鉴权事实节）|
...
## 2. 鉴权事实节填写指引（analysis-auth.md 第一节，结构化）

读源码后，把下列**事实**填成 `<源项目>/.ai-mode-skills/analysis-auth.md` 第一节（表格即可，不必 JSON）。每条事实必须带 `来源 = 文件:行号`。不论链式 builder / 框架封装 / 裸 `wx.request`，都归约到下面这些维度。
...
|----|----|----|
| `<hostId>` | `https://...` | `<文件>:<行号>` |
| `_inline` | （整 URL 在调用处硬写，analysis-apis.md 直接记完整 url）| |
...
- `scope=per-interface`：签名盐值（salt）/输入逐接口不同时，analysis-apis.md 用 `kind:computed` 引用本方案 + 给本接口的具体 salt 表达式。
- 无签名 = 空表。
...
| `dynamic` | JSAPI / 登录态（如定位坐标、token） | **必须生成 `ensure<Name>()` 函数**，实调取真值并 `await`（§4 要点） |
| `userInput` | 用户输入 | 从 `inputSchema` 传入 |
| `upstreamRequest` | 上游接口下发的 token/seed | 上游接口 `structuredContent` 传下来，在 `inputSchema` 声明 |

---

## 3. 逐字代码片段节（analysis-auth.md 第二节，原样拷贝）

凡 §2 表格里 `原文片段` / `依赖模块` 指向的代码，**原文照搬**到 analysis-auth.md 第二节（压缩代码也照抄，不格式化成"步骤"），代码生成时直接保留/内联，不重写：

- 签名/反爬函数本体
- 表格中列出的依赖模块（**整文件**）
- 请求封装闭包里通用参数注入的那一段（若 `hasCentralWrapper=true`）

每段标 `#anchor` + 来源 `文件:行号`，供 §2 引用。

---

## 4. JSAPI 动态值获取要点（逐字复刻主包链路，不得简化）
...
- [ ] 登录换取接口的 URL / host / 请求字段名 / method / 响应取值路径与 analysis-auth.md、源码一致（重点核对：登录 code 类字段别名、响应嵌套层级、请求/响应字段不同名）
...
- [ ] analysis-auth.md 标为需登录态的每个 `apis/*.js`，入口都 `await ensureLogin()` 且跑完了 §2.5 整条多级链路（无半程、无外部注入占位）
